...
$app->group('/v1', function () use ($app) {
    ...
    * Expects a "format" parameter ("xml"/"html"/"scorm") that describes the format of learning module's content
     */
    $app->get('/courses/download/lm/:ref_id', RESTAuth::checkAccess(RESTAuth::PERMISSION), function ($ref_id) use ($app) {
        $request = $app->request();
        $format = $request->params("format");
        if($format != "xml" && $format != "html" && $format != "scorm")
            $app->halt(400, "Unsupported or missing 'format' parameter. Use eiter 'xml' or 'html' or 'scorm'");
        try {
            $exp = (new CoursesModel())->downloadLearningModule($ref_id, $format);
            $app->success(array('path' => $exp['directory']."/".$exp['file'], 'file' => $exp['file']));
        } 
        catch(Courses\CancelationFailed $e) {
            $app->halt(400, 'Error: Could not perform action download lm.');
        }
    });
});
